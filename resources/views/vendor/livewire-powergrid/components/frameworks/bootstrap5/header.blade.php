<div>
    ...
    @php
        $hasHeaderControls = data_get($setUp, 'exportable')
            || data_get($setUp, 'header.toggleColumns')
            || data_get($setUp, 'header.softDeletes')
            || boolval(data_get($setUp, 'header.wireLoading'))
            || (config('livewire-powergrid.filter') === 'outside' && count($this->filters()) > 0)
            || data_get($setUp, 'header.searchInput');
    @endphp

    @if ($hasHeaderControls)
        <div class="dt--top-section app-grid-header">
            <div class="row g-3 align-items-center">
                ...
                <div class="col-12 col-sm-6 d-flex justify-content-sm-end justify-content-center mt-sm-0">
